...
(* :Copyright: Copyright 1992-2003,  Mark Sofroniou.
...
(* :History:
 December 14 2004 - Added an improvement to treat Power[x, 2] as x*x
 rather than e.g. pow(x, 2.) in C. Made expression optimization
 the default for CAssign and FortranAssign and improved the error
 handling when expression optimization fails.

 December 13 2004 - Fixed a bug in formatting nested powers,
 reported by Frank Kampas.

 December 4 2004 - Fixed a bug when no expression optimization occurs,
 reported by Frank Kampas.

 October 18 2004 - Fixed a bug in displaying zeros in FortranAssign.

 May 2003 - Changes for compatibility with Version 5.0 of Mathematica.

 March 1997 - Modified syntax of expression optimization.

 October 1995 - Modified powers of rationals.
 ...
 as AssignToArray. Improved line breaking and option/argument
...
(* :Source:
 Mark Sofroniou, Symbolical and Numerical Methods for Hamiltonian Systems,
 Ph.D. Thesis, Loughborough University, Loughborough,
 Leicestershire LE11 3TU, United Kindom. *)
...
sequence. The degree of optimization performed can be set as options to
the function Experimental`OptimizeExpression. AssignOptimize may evaluate
to True or False."

AssignPrecision::usage = "AssignPrecision specifies the precision
of real numbers in expressions. Its value may be any positive number
or infinity."
...
User definitions for a symbol may interfere with the assignment process.
...
AssignOptimize::fail = "Invalid options for OptimizeExpression.
Continuing with unoptimized expression."
...
PositiveNumberQ[x_]:= NumberQ[x] && TrueQ[Positive[x]];
...
errmsgs = {
  ...
  {"option AssignPrecision","a positive number or infinity"},
...
MatchQ[prec,_?PositiveNumberQ|Infinity|MachinePrecision],
...
]& @ (Experimental`OptimizationSymbol /. {opts} /. Options[Experimental`OptimizeExpression]);
...
AssignOptimize->True, AssignPrecision->MachinePrecision,
...
(* Add remaining formatting rules. These are applied here to avoid
 any conflict with code optimization. *)
        ...
        powerrules = {
          ...
          Power[a_, r:Rational[_,_]]:> pow[a,N[r]],
          (* Squares *)
          Power[a_Symbol, 2]:> HoldForm[a*a],
          (* Remaining powers. *)
          Power[a_, b_?(NumericQ[#]&&#!=-1&)]:> pow[a,N[b,prec]],
...
AssignMaxSize->5000, AssignOptimize->True,
AssignPrecision->MachinePrecision, AssignRange->False,
...
(* Output a list as a column and avoid string delimiters "". *)
      ...
      OutputForm[ ColumnForm[ Flatten[newexpr] ] ]
...
Literal[FRH[Sign[x_]]]:= FD[Sign][1,x];
...
(* Add remaining formatting rules. These are applied here to avoid
 any conflict with code optimization. *)
        ...
        powerrules = {
          ...
          Power[a_, r:Rational[_,_]]:> a^N[r, prec],
		  (* Squares *)
          Power[a_Symbol, 2]:> HoldForm[a*a]};
...
Assign[lhs_:"",expr_?(!OptionQ[#]&),form_Symbol,opts___?OptionQ]:=
  Module[{optvals},
    optvals /;
      And[
...
HoldPattern[
  CommonAssign[
    lhs_, Experimental`OptimizedExpression[Block[olhs_,optseq__; oexpr_]],
      form_, args__
  ]
]:=
  With[{ orhs = Block[{Set}, Map[ Last, Unevaluated[{optseq}] ] ]},
    CommonAssign[ Join[Makelhs[olhs,form],{lhs}], Join[orhs,{oexpr}], form,
      args ]
  ];
...
(* Output results to a file. *)
    ...
    If[atofile=!="",
      ...
      Write[outchan, ColumnForm[strings]];
      Close[outchan] ];
...
MyN[expr_,prec_,{}]:=
  If[SameQ[prec, MachinePrecision],
    #,
...
AssignOpt[expr_,optopts___?OptionQ]:=
  CheckOptimization[ Experimental`OptimizeExpression[expr, optopts], expr ];

(* Check for optimization. *)

CheckOptimization[expr_, expr_]:= expr;

(* Issue message and proceed with unoptimized expression. *)
CheckOptimization[_Experimental`OptimizeExpression, expr_]:= (Message[AssignOptimize::fail]; expr);

CheckOptimization[Experimental`OptimizedExpression[expr_], expr_]:= expr;

CheckOptimization[optexpr_, expr_]:= optexpr;
