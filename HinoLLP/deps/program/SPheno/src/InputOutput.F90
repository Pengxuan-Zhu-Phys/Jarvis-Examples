...
! Florian Staub
    ...
    Else If (read_line(7:11).Eq."GAUGE") Then
     If (external_spectrum) Call Read_Gauge(99)
  ...
  Implicit None
   ...
   Real(dp) :: wert2
    Do 
     ...
     Case default
      Write(ErrCan,*) "Read_Gauge, Parameter with id=",i1," is unknown"
...
!       If (i_c.Eq.1) Alam12_0 =  Cmplx(Real(Alam12_0,dp) , wert, dp)
 ...
 Implicit None
  ...
  !---------------------------------------------------------------------
  ! variables for check if neutralino is the LSP
  !---------------------------------------------------------------------
  Real(dp) :: m_c, m_lsp
  Logical :: l_c_lsp
...
!   L_BR = .False.
  ...
  !-----------------------
   ...
   !-----------------------------------------------
   ! check if the lightest neutralino is the LSP
   !-----------------------------------------------
   l_c_lsp = .False.
   If (HighScaleModel.Eq."NMSSM") Then
    m_lsp = Minval(Abs(Chi05(:)%m))
   Else If (HighScaleModel.Eq."RPexplicit") Then
    m_lsp = 0._dp
   Else 
    m_lsp = Minval(Abs(Chi0(:)%m))
   End If
   m_c = Minval(Abs(ChiPm(:)%m))
   If (m_c.Lt.m_lsp) l_c_lsp = .True.
   m_c = Minval(slepton(:)%m)
   If (m_c.Lt.m_lsp) l_c_lsp = .True.
   m_c = Minval(SUp(:)%m)
   If (m_c.Lt.m_lsp) l_c_lsp = .True.
   m_c = Minval(SDown(:)%m)
   If (m_c.Lt.m_lsp) l_c_lsp = .True.
   If (m_lsp.gt.Glu%m) l_c_lsp = .True.
   If (l_c_lsp) Write(io_L,100) &
      & "     3               # charged LSP, please check block MASS for its identity"
