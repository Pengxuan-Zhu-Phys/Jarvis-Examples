...
public:
   ...
   std::tuple<lowMSSM<T>> get_models_slha() const
   { return std::make_tuple(lowMSSM<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const lowMSSM<T>& get_model() const
   { return model; }
   lowMSSM<T>& get_model()
   { return model; }
   lowMSSM<T> get_model_slha() const
   { return lowMSSM<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   double start, double stop) const
{
   ...
   auto data_getter = [&tmp_model](double scale) {
      ...
      return lowMSSM_parameter_getter::get_parameters(tmp_model);
   };

   std::vector<std::string> parameter_names(
      std::cbegin(lowMSSM_info::parameter_names),
      std::cend(lowMSSM_info::parameter_names));
