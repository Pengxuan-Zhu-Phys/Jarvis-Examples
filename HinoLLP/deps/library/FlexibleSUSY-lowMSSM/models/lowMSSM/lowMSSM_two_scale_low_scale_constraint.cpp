...
double lowMSSM_low_scale_constraint<Two_scale>::calculate_theta_w()
{
   ...
   const auto get_mw_pole = [&] () {
      double mw_pole = MODEL->get_physical().MVWm;
      if (mw_pole == 0) {
   ...
   lowMSSM_weinberg_angle::Parameters parameters;
   ...
   lowMSSM_weinberg_angle weinberg(MODEL, parameters);
   ...
   try {
      ...
      if (MODEL->get_thresholds() > 0 && MODEL->get_threshold_corrections().
         sin_theta_w > 0) {
