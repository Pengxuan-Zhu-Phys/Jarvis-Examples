...
Run the migrated SUSYHIT CSV replay card to verify the external-program call path:

```bash
Jarvis bin/SUSYHIT_Calculator_Validated_CSV.yaml --skip-draw-flowchart
```

Run the migrated Bridson scan card:

```bash
Jarvis bin/Bridson_Higgsino_LLP_muTB_M2.yaml
```

The original scaffold examples are still available:
...
- `deps/program/susyhit/`: migrated SUSYHIT source archive and input templates
- `calculators/program/`: SUSYHIT install/execution directories created by the YAML cards
...
Add optional project directories such as `configs/`, `scripts/`, `assets/`, and `docs/` only when your workflow actually needs them.
